...
<div class="col-lg-12">
    ...
    <div class="card">
      <div class="card-body"><br>
        <form class="row g-3" action="{{ route('updatePengiriman', $data->id) }}" method="POST">
        @csrf
          <div class="col-12">
            <label for="id_produksi" class="form-label">Nama Sepatu</label>
              <select id="id_produksi" class="form-control" name="id_produksi" required>
                ...
                @foreach ($data_produksi as $row) 
                  <option value="{{ $row->id }}" {{ $row->id == $data->id_produksi ? 'selected' : '' }}>{{ $row->nama_sepatu }}
                @endforeach
              </select> 
          </div>
          <div class="col-12">
            <label for="penerima" class="form-label">Penerima</label>
            <input type="text" class="form-control" id="penerima" name="penerima" value="{{ $data->penerima }}" required>
          </div>
          <div class="col-12">
          ...
          <div class="col-12">
            ...
            <input type="text" class="form-control" id="alamat_tujuan" name="alamat_tujuan" value="{{ $data->alamat_tujuan }}" required>
          </div>
          <div class="col-12">
            <label for="status_pengiriman" class="form-label">Kategori Tiket</label>
              <select class="form-select" id="status_pengiriman" name="status_pengiriman"  value="{{ $data->status_pengiriman }}" required>
